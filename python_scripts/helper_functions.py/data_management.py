...
def under_sample_negative_class(data_array: np.ndarray) -> np.ndarray:
    ...
    final_negative_array, _ = sample_data(negative_array, 'uniform', percentage_to_sample)

    # Get the undersampled data
    undersampled_data = np.concatenate((data_array[pos_indices, :], final_negative_array))

    return shuffle(undersampled_data)
...
def random_sampling(
    ...
    if undersample:
        train_array = under_sample_negative_class(train_array)
        val_array = under_sample_negative_class(val_array)
        test_array = under_sample_negative_class(test_array)
